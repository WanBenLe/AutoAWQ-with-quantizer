...
from awq.utils.calib_data import get_calib_dataset, custom_multimodal_dataset
...
class AwqQuantizer:
    ...
    def _search_best_scale(
        ...
        with torch.no_grad():
            ...
            print(inp.size())
            print('inp inf',torch.isinf(inp).sum())
            fp16_output = module2inspect(inp, **module_kwargs)
            
            if isinstance(fp16_output, tuple):
                fp16_output = fp16_output[0]
            print('fp16_output inf',torch.isinf(fp16_output).sum())
        # [STEP 4]: Compute loss
    ...
    def _compute_best_scale(
        ...
        if best_ratio == -1:
            print('x',x)
            print('int_w_output')
            print(torch.isinf(int_w_output).sum())
            print('fp16_output')
            print(torch.isinf(fp16_output).sum())
            print(loss)
            print((fp16_output - int_w_output).float().mean().item())
    ...
    def _get_input_feat(self, layer, named_linears):
        ...
        if self.inps is not None:
            ...
            print('self.inps',self.inps)
            print('self.inps',self.inps.size())
        else:
            self.inps=self.module_kwargs['inputs_embeds'].to(torch.float16).to(next(layer.parameters()).device)
            print('self.inps',self.inps)
            print('self.inps',self.inps.size())
